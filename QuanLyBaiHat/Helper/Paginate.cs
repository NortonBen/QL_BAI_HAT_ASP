...
namespace QuanLyBaiHat.Helper
{
    public class Paginate
    {
        ...
        public static string renderForm(Dictionary<string, int> data)
        {
            bool next = false;
            bool prev = false;
            List<int> show_page = new List<int>();
            ...
            show_page.Sort();
            string _header = "<nav><ul class=\"pagination\">";
            string _prev = " <li><button type=\"submit\" name=\"page\" value =\"" + (data["page"] - 1).ToString() + "\" class=\"waves-effect\"><i class=\"material-icons\">chevron_left</i></button></li>";
            string _str = "";
            foreach (int i in show_page)
            {
                _str += "<li " + ((data["page"] == i) ? "class=\"active\"" : "") + " ><button type=\"submit\" name=\"page\" value =\"" + (i).ToString() + "\" class=\"waves-effect\">" + (i + 1).ToString() + "</button></li>";

            }
            string _next = "<li><button type=\"submit\" name=\"page\" value =\"" + (data["page"] + 1).ToString() + "\" class=\"waves-effect\"><i class=\"material-icons\">chevron_right</i></button></li>";
            string _footer = "</ul></nav>";
            if (prev)
            {
                _header += _prev;
